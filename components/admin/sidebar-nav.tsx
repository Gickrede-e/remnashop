...
export function AdminSidebar() {
  ...
  return (
    <aside className="page-surface h-fit p-3 lg:p-4">
      <p className="hidden px-3 pb-3 text-xs uppercase tracking-[0.24em] text-zinc-500 lg:block">Админ-панель</p>
      <nav className="-mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-1 lg:mx-0 lg:grid lg:gap-2 lg:overflow-visible lg:px-0 lg:pb-0">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex min-h-11 min-w-max shrink-0 snap-start items-center gap-3 rounded-2xl px-4 py-3 text-sm text-zinc-400 transition-colors hover:bg-white/[0.04] hover:text-white lg:px-3",
